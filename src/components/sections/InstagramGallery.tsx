...
export function InstagramGallery() {
  return (
    <section id="bean-beyond" className="bg-white px-4 py-16 text-center sm:px-6 sm:py-24 lg:px-8">
      <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.2 }} variants={stagger} className="mx-auto max-w-7xl text-center">
        ...
        <motion.div variants={stagger} className="mt-14 grid gap-4 text-center sm:grid-cols-2 lg:grid-cols-6">
          {gallery.map((image, index) => (
            ...
            >
              <img src={image} alt="My Coffee Co. lifestyle gallery" className={`w-full object-cover transition duration-700 group-hover:scale-110 ${index === 0 || index === 3 ? "h-72 sm:h-96 lg:h-[420px]" : "h-64 sm:h-52"}`} />
